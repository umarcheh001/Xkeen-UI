...
Исправление ограничения resize 29 июля 2026 года: удалены presentation-лимиты `760px`/`90vh`, из-за которых центральный grip продолжал двигаться, но высота окна переставала расти. Пользовательская высота может превышать viewport на высоких экранах (до защитного предела 4096 px), но операторский слой ограничивает карточку высотой доступной области вкладки. Это сохраняет нижнюю панель действий видимой даже при старой сохранённой геометрии и оставляет resize-grip рабочим на больших окнах.

Стабилизация геометрии 7 сентября 2026 года: горизонтальное растягивание отключено. Карточка всегда следует ширине рабочей области, а в localStorage сохраняется только высота. Старые значения `w`/`shiftX` игнорируются и удаляются из inline-стилей при инициализации, поэтому растягивание влево больше не может спрятать карточку и её ручку за экраном. Остаётся только вертикальный resize-grip.
...
- desktop fullscreen и lite-mode ограничения сохранены;
- ширина карточки фиксирована относительно рабочей области; вертикальный bottom grip изменяет и сохраняет только высоту;
