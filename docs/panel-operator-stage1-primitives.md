...
Значения выше являются историческим snapshot закрытия Этапа 1, а не бессрочным потолком для следующих экранов. С 29 июля 2026 года дальнейшие этапы используют living budget для всего `panel-operator.css`:

- selector definitions: не больше 1200;
- unique selectors: не больше 1000;
- selectors с повторным определением: не больше 145;
- дополнительные повторные instances: не больше 190.

Лимиты definitions/unique расширены 29 июля 2026 года после закрытия задачи Logs: слой достиг 925 / 782 при сохранённых 128 повторных selectors и 143 дополнительных instances. Расширение устраняет искусственные `:is()`-переписывания ради счётчика и оставляет запас для Этапов 4–5; лимиты повторов намеренно не повышены.

Это потолки регрессии, а не целевые значения. Они не отменяют более важные guardrails: один канонический scoped-слой, отсутствие правил после responsive, отсутствие второго набора geometry primitives и сохранение замороженного legacy boundary.
