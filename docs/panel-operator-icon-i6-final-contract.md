...
Проверка закрытия: `frontend:verify`, 18 статических контрактных проверок и выделенный Chromium-набор I5/Stage 0 (11 сценариев) проходят. `scripts/run_e2e_server.py` теперь перед стартом создаёт чистый изолированный runtime с fixture-конфигами для Xray, Mihomo, списков Xkeen и UI settings; Playwright также создаёт новую auth storage state для каждого запуска. Это устраняет утечку локального роутерного состояния, но полный E2E пока не является gate: при последовательном запуске остаются order-dependent сценарии (routing schema/editor и часть smoke/visual tests), поэтому для полного gate нужна отдельная изоляция состояния на тест/файл.

Sprite/helper изменились в I6, поэтому cache-buster обновлён на `20260802a`; финальный router archive пересобран. Runtime hooks, маршруты, `id`, `data-*` и обработчики не изменялись.
