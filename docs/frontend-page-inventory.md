...
- top-level переходы между `/`, `/devtools` и `/mihomo_generator` идут через фиксированный route map и `pushState`/`popstate`, а hard navigation остаётся только fallback-path;
