...
- В **«Соединениях»** и рядом с IP-адресами в **«Логах»** показываются имена устройств из списка Keenetic. Используется тот же источник ручных и роутерных имён, что и в логах Xray, с общим коротким кэшем без запроса к роутеру на каждую строку.

- Экран **«Соединения»** получил вкладку недавно закрытых соединений (до 300 записей только в памяти браузера), расширенные безопасные метаданные, сортировку по заголовкам, быстрый фильтр по клику и копирование отдельных значений или всей карточки.
- У rule-provider появилась кнопка read-only просмотра содержимого: inline, YAML, Text и MRS открываются внутри вкладки **«Правила»**, поддерживают поиск и страницы. Чтение ограничено по размеру и числу правил; пути не принимаются от браузера и не раскрываются, MRS обрабатывается во временном каталоге, результаты кэшируются по времени изменения файла.
