...
| Mihomo workflows | Routing endpoints, read-only `/api/mihomo-templates` / `/api/mihomo-template` и точечные utility endpoints | Routing и выбор шаблона уже подключены; остальная поверхность слишком широка | Следующими slices переносить `Узел` и `HWID`; `Zashboard UI` открывать внешним браузером, а профили, подписки, бэкапы и генератор не переносить |
