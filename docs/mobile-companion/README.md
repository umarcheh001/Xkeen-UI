...
- Из текущей карты Mihomo placeholder-границами остаются `Узел` и `HWID`.
...
- Точечные Mihomo-инструменты `Узел` и `HWID` по отдельным безопасным контрактам; `Шаблоны` и внешнее открытие `Zashboard UI` уже реализованы.
...
- `Узел` и `HWID` как следующие отдельные Mihomo-срезы; `Шаблоны` и внешнее открытие `Zashboard UI` уже реализованы.
