...
Главный технический разрыв сейчас не в создании Android UI, persistence подключений или encryption at rest, а в переходе к реальному transport, backend-validated session restore и write-safe mobile contract.
Базовый controller уже переведен на dependency seam через `ConnectionsPort`, `SessionPort`, `ServiceActionsPort`, `RoutingWritePort` и `LogsPort`; connection и session-material storage уже имеют persisted implementation, остальные адаптеры будут заменяться по следующим этапам.
...
- Нет реального auth/session bootstrap и backend-validated trusted session restore; secure storage готово, но текущая demo-сессия всегда помечается `trustedForRestore = false`.
