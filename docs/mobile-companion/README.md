...
- `Шаблоны Mihomo` используют существующие web endpoints списка/чтения/сохранения. Выбранный YAML сначала попадает только в мобильный draft редактора; `Zashboard UI` открывается внешним браузером без передачи ему session material приложения.
- `Узел Mihomo` использует `POST /api/mihomo/node/import-draft`: нативный экран выбирает source type и `proxy-groups`, сервер возвращает изменённый draft без записи файла, а общий YAML-редактор выделяет вставленный блок. Xray JSON-подписки могут быть зарегистрированы для автообновления.
- `Shell` содержит журнал событий приложения и рабочий терминал.
- Из текущей карты Mihomo placeholder-границей остаётся `HWID`.
...
- Следующий точечный Mihomo-инструмент — `HWID`; `Узел`, `Шаблоны` и внешнее открытие `Zashboard UI` уже реализованы.
...
- `HWID` как следующий отдельный Mihomo-срез; `Узел`, `Шаблоны` и внешнее открытие `Zashboard UI` уже реализованы.
