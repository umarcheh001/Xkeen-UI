...
Главный технический разрыв теперь находится в transport для streams. Controller использует dependency seam через `ConnectionsPort`, `SessionPort`, `ServiceActionsPort`, `RoutingValidationPort`, `RoutingWritePort` и `LogsPort`; все перечисленные порты, кроме logs, уже имеют production implementations.
