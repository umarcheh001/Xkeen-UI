...
Главный технический разрыв сейчас не в создании Android UI или persistence подключений, а в переходе к реальному transport, secure session model и write-safe mobile contract.
Базовый controller уже переведен на dependency seam через `ConnectionsPort`, `SessionPort`, `ServiceActionsPort`, `RoutingWritePort` и `LogsPort`; первый порт уже заменен на persisted implementation, остальные адаптеры будут заменяться по следующим этапам.
...
- Storage: видимые данные подключения хранятся в app-private storage, а token/cookie/password и trusted-restore marker должны жить только в отдельном secure storage.
...
- Нет реального auth/session bootstrap, secure storage для session material и trusted session restore.
