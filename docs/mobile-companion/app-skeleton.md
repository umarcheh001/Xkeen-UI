...
- `Роутинг Mihomo`, `Шаблоны` и `Узел` используют реальные backend-контракты; `Zashboard UI` открывает прямой адрес Mihomo `http://<адрес Xkeen-узла>:9090/ui` в системном браузере.
- `Узел` использует нативный source/group flow без отдельного YAML preview и передаёт draft в общий редактор с выделением вставленного блока. В Mihomo placeholder-контрактом остаётся `HWID`; `Ports` использует единый рабочий редактор файлов портов и исключений.
