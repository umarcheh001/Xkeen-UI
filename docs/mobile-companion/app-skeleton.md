...
- `Роутинг Mihomo` и `Шаблоны` используют реальные backend-контракты; `Zashboard UI` открывает прямой адрес Mihomo `http://<адрес Xkeen-узла>:9090/ui` в системном браузере.
- В Mihomo placeholder-контрактом для следующих модулей остаются `Узел` и `HWID`; `Ports` использует единый рабочий редактор файлов портов и исключений.
