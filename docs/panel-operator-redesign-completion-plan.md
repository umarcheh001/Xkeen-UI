...
Раскрытая группа использует рабочую ширину как адаптивную 7/6/5/4/3/2/1-колоночную сетку узлов. Отдельный текстовый столбец «Состояние» удалён как дублирующий: доступность кодируется цветом маркера у задержки, недоступный узел получает явное значение «недоступен», а текстовое состояние сохранено в доступном имени и tooltip задержки.
...
- [x] обновить cache-buster `panel-operator.css` до `20260810d`;
