...
Визуальная доводка Mihomo после проверки 29 июля 2026 года: с отдельного route генератора сняты legacy blue-glass header/lead/status/log surfaces; обе рабочие колонки ограничены доступной высотой viewport с внутренней прокруткой, а preview остаётся единственным растущим регионом. Результаты проверки в **Mihomo Generator** и в **Роутинге Mihomo** используют одну flat diagnostic grammar: auto-height на desktop, нейтральные `--op-*` surfaces, semantic state без blue-glass/pills и fullscreen с единственным scroll-region на mobile. Bulk Import больше не резервирует фиксированный canvas 760 px, Premium Import/HWID используют preview 240–360 px, профили и бэкапы разложены полноширинными таблицами, а имя нового профиля получило собственную inline error-note. Scoped layer по-прежнему загружается последним без изменения API и runtime hooks.
...
Последующая визуальная доводка: массовый control явно остаётся в одной строке с пиктограммой (как «Тест видимых»), а между workspace bar и runtime card восстановлен самостоятельный отступ. При первом ответе API все группы добавляются в collapsed-state; «Развернуть» становится явным следующим действием и проверка видимых узлов остаётся неактивной, пока пользователь не раскроет нужную группу.

Раскрытая группа переиспользует проверенный паттерн карточек из модала подписок Xray: адаптивный `auto-fill` с минимальной шириной карточки 232 px, самостоятельные границы и интервалы, прозрачный canvas без серых пустых ячеек, действия справа и задержка в компактном tone-badge. Отдельный текстовый столбец «Состояние» удалён как дублирующий: недоступный узел получает явное значение «недоступен», а текстовое состояние сохранено в доступном имени и tooltip задержки.
...
- [x] по умолчанию сворачивать все группы; предоставить явные «свернуть/развернуть» без скрытия поиска;
