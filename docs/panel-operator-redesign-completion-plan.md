...
## Исторический ledger закрытых этапов

Следующие формулировки были contract-маркерами предыдущей версии плана и сохранены здесь намеренно. Они описывают исторические даты закрытия работ, а не заменяют актуальную таблицу статусов выше. Контрактные тесты документации используют этот ledger, чтобы обновление плана не стирало историю решений и ссылок на закрытые этапы.

- `Параллельный проход: DevTools — закрыт 29 июля 2026 года` — отдельный DevTools Operator Console pass.
- `Этапы 0–3 закрыты 28 июля 2026 года; Этап 4 в работе: задачи «Порты», «Routing rules» и «Balancers» закрыты 28 июля 2026 года` — исходная отметка промежуточного аудита.
- `### Этап 0. Зафиксировать контракт и матрицу состояний — закрыт`; критерий завершения: **выполнен**; источник — `panel-operator-stage0-inventory.json`.
- `### Этап 1. Уплотнить scoped-слой в систему примитивов — закрыт`; критерий завершения: **выполнен**; источник — `panel-operator-stage1-primitives.md`.
- `### Этап 2. Шапка, навигация и рабочая сетка — закрыт`; критерий завершения: **выполнен**; источник — `panel-operator-stage2-shell-grid.md`.
- `### Этап 3. Пересобрать routing cards и operational blocks — закрыт`; критерий завершения: **выполнен**; источник — `panel-operator-stage3-routing-cards.md`.
- Историческая запись критериев закрытия ранних этапов: `Критерий завершения: **выполнен**`.
- Исторический рабочий заголовок: `### Этап 4. Формы, таблицы и data-heavy экраны — в работе`; в итоге `Этап 4 закрыт 29 июля 2026 года`.
- `- [x] **Порты:**`, `- [x] **Routing rules:**`, `- [x] **Balancers:**`, `- [x] **Commands:**`, `- [x] **Logs:**`, `- [x] **Files:**`, `- [x] **Mihomo profiles/generator:**`, `- [x] **Формы подписок:**` — закрытые партии Этапа 4.
- **Визуальная доводка Mihomo** — историческое название дополнительного прохода после закрытия форм и генератора; его результаты теперь входят в Этап C нового плана.
- `задача «Порты» Этапа 4 закрыта 28 июля 2026 года`; `задача «Routing rules» закрыта`; `задача «Balancers» закрыта`; `задача «Commands» закрыта 29 июля 2026 года`; `Задача «Files» закрыта 29 июля 2026 года`; `задача «Logs» закрыта 29 июля 2026 года`.
- Исторический заголовок Этапа 5: `### Этап 5. Редакторы и модальные семейства — в работе`; Семь связанных задач закрыты 2–4 августа 2026 года, источник — `panel-operator-stage5-editor-workbench.md`.
- Закрытые исторические задачи editor/workbench: `[x] создать единый editor modal contract для JSON, file editor и snapshot`; `[x] заменить comments/schema pills на компактные status labels`; `[x] применить четыре modal family ко всем 50 окнам в приоритетном порядке из аудита`; `[x] убрать чисто презентационные inline max-width/gap/margin после переноса в scoped classes;`; `[x] для пустых/error состояний использовать auto-height вместо искусственно высокого body;`.
- Исторический раздел: `## 6. Presentation geometry и empty/error auto-height — закрыто`; пустые error placeholders, `.hidden`, `[hidden]` и `display:none` не активируют этот режим; критерий этих семи задач выполнен.
