...
def create_backups_blueprint(
    ...
    def _main_json_for(path: str) -> str:
        """Normalize a selected path to the main .json file."""
        try:
            p = str(path or "")
        except Exception:
            p = ""
    ...
    def _read_raw_jsonc_text(main_json: str) -> Optional[str]:
        """Read raw JSONC from JSONC_DIR, with legacy fallback."""
    ...
    def _resolve_any_fragment_basename(name: str) -> Optional[str]:
        """Resolve fragment path by basename within configs dir (no kind restriction)."""
        bn = safe_basename(name)
        if not bn:
            return None
        lname = bn.lower()
        if not (lname.endswith(".json") or lname.endswith(".jsonc")):
            return None
        if lname.endswith(".jsonc"):
            bn = bn[:-1]
        return safe_child_realpath(_CONFIGS_DIR, bn)

    def _extract_target_file_from_backup(data: Any) -> Optional[str]:
        """Return target config file path from backup metadata, if present."""
        if not isinstance(data, dict):
            return None
        meta = data.get("__xkeen_target_file__")
        if not meta:
            return None
        try:
            return _resolve_any_fragment_basename(str(meta))
        except Exception:
            return None
    ...
    def api_create_backup() -> Any:
        ...
        raw_text = _read_raw_jsonc_text(routing_main)
    ...
    @bp.post("/api/backup-fragment")
    def api_create_backup_fragment() -> Any:
        """Create manual backup of any Xray fragment file.

        Required query param: ?file=<basename>.json

        Backup payload is a wrapper with metadata:
          - __xkeen_target_file__: basename of the fragment to restore into
          - __xkeen_raw_jsonc__: raw JSONC (if exists in JSONC_DIR)
          - __xkeen_data__: cleaned JSON object (if parsable)
        """
        ...
        selected = _resolve_any_fragment_basename(file_arg)
        if not selected:
            return jsonify({"ok": False, "error": "bad file"}), 400

        main_json = _main_json_for(selected)
        if not os.path.isfile(main_json):
            return jsonify({"ok": False, "error": "file not found"}), 404

        raw_text = _read_raw_jsonc_text(main_json)
        data = load_json(main_json, default=None)

        if raw_text is None and data is None:
            return jsonify({"ok": False, "error": "file invalid"}), 400

        if not os.path.isdir(BACKUP_DIR):
            os.makedirs(BACKUP_DIR, exist_ok=True)

        ts = time.strftime("%Y%m%d-%H%M%S")
        base = os.path.basename(main_json)
        prefix = os.path.splitext(base)[0] or "fragment"
        fname = f"{prefix}-{ts}.json"
        path = os.path.join(BACKUP_DIR, fname)

        payload: dict[str, Any] = {"__xkeen_target_file__": os.path.basename(main_json)}
        if raw_text is not None:
        ...
        _core_log(
            ...
            kind="fragment",
            filename=fname,
            file=os.path.basename(main_json),
            remote_addr=str(request.remote_addr or ""),
        )
        return jsonify({"ok": True, "filename": fname}), 200
    ...
    def api_restore_backup() -> Any:
        ...
        # Backups may include raw JSONC with comments.
        if isinstance(data, dict) and "__xkeen_raw_jsonc__" in data:
            ...
            # If it's a wrapper payload, prefer __xkeen_data__.
            if isinstance(data, dict) and "__xkeen_data__" in data:
                save_json(target_file, data.get("__xkeen_data__"))
            else:
    ...
    @bp.post("/api/restore-auto-fragment")
    def api_restore_auto_backup_fragment() -> Any:
        """Restore the selected fragment from the latest auto-backup.

        Body: {"file": "<basename>.json"}
        """
        payload = request.get_json(silent=True) or {}
        file_arg = (payload.get("file") or "").strip()
        if not file_arg:
        ...
        config_path = _resolve_any_fragment_basename(file_arg)
        if not config_path:
            return jsonify({"ok": False, "error": "bad file"}), 400

        config_path = _main_json_for(config_path)
        ...
        # If auto backup contains raw JSONC, restore it too.
        if isinstance(data, dict) and "__xkeen_raw_jsonc__" in data:
            raw = data.get("__xkeen_raw_jsonc__") or ""
            try:
                ensure_xray_jsonc_dir()
            ...
            if isinstance(data, dict) and "__xkeen_data__" in data and "__xkeen_target_file__" in data:
                save_json(config_path, data.get("__xkeen_data__"))
            else:
                save_json(config_path, data)
        ...
        return jsonify({"ok": True, "filename": filename, "file": os.path.basename(config_path)}), 200
