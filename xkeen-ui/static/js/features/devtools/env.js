...
(() => {
  ...
  const ENV_HELP = {
    ...
    'XKEEN_XRAY_JSONC_DIR': 'Каталог, где UI хранит raw JSONC sidecar файлы (с комментариями). По умолчанию: /opt/etc/xkeen-ui/xray-jsonc. Важно: должен быть вне XKEEN_XRAY_CONFIGS_DIR, чтобы Xray случайно не пытался парсить *.jsonc.',
    ...
    'XKEEN_XRAY_ROUTING_FILE_RAW': 'Путь или имя raw JSONC-файла роутинга (с комментариями). Если указано имя без / — считается относительно XKEEN_XRAY_JSONC_DIR. Если не задано — используется <routing>.jsonc в XKEEN_XRAY_JSONC_DIR.',
  ...
  ENV_RESTART_KEYS.delete('XKEEN_MIHOMO_UI_ALLOW_PROXY_FALLBACK');
