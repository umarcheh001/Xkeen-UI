  ...
  function describeState(data, flags) {
    ...
    if (flags.enabled && unavailable) {
      return {
        ...
        text: installed
          ? 'Служебная конфигурация сохранена, однако процесс Xray сейчас не найден и DNS через туннель не отвечает. Переустанавливать ядро не нужно: проверьте состояние XKeen и журнал ошибок Xray. Сторож вернёт DNS роутеру, если сбой сохранится. Ждать его не обязательно: кнопка ниже снимет настройку DNS-over-VLESS, а вместе с ней уйдёт и фрагмент, из-за которого ядро может не подниматься.'
