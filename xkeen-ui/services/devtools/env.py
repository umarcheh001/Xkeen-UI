    ...
    *,
    ...
    if k == "XKEEN_HAPP_DECRYPTOR_TIMEOUT":
        try:
            ...
            return str(int(happ_links.default_decryptor_timeout_seconds()))
        except Exception:
            return "45"
    if k == "XKEEN_HAPP_DECRYPTOR_RELEASE_URL":
        from services.happ_decryptor import engine as happ_engine

        return happ_engine.DEFAULT_RELEASE_URL
    if k == "XKEEN_HAPP_KEYS_MANIFEST_URL":
...
def write_env_file(path: str, values: Mapping[str, str]) -> None:
    ...
    lines = ["# Generated by Xkeen UI DevTools", "# Format: export KEY='value'", ""]
    for k in sorted(values.keys()):
        v = values.get(k, "")
        lines.append(f"export {k}={_shell_quote_single(str(v))}")
    # Общая атомарная запись вместо своей: прежняя брала имя «путь + .tmp» и
    # ломалась, когда файл писали двое сразу.
