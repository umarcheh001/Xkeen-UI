...
def _is_generated_jsonc_comment(text: str, header: str = "") -> bool:
    raw = str(text or "")
    if "Generated by XKeen UI subscription" in raw or "Generated by XKeen UI subscriptions" in raw:
        return True
    # The header we are about to write again is ours, not the user's. Without this the
    # previous header survives as an ordinary comment and a fresh one lands above it,
    # so the file grows by one line on every write.
    head = str(header or "").strip()
    return bool(head) and raw.strip() == head
...
def _collect_jsonc_comments_by_semantic_key(text: str, root_obj: Any, *, header: str = "") -> Dict[str, List[str]]:
    ...
    def add_comment(path: str, comment: str) -> None:
        raw = str(comment or "")
        if not raw.strip() or _is_generated_jsonc_comment(raw, header):
            return
