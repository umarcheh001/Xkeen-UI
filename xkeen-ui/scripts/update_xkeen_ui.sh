...
download() {
  url="$1"; out="$2"; max_bytes="${3:-}"; label="${4:-file}"
...
LATEST_OUT="$($PY - "$API_BASE" "$REPO" "$API_TIMEOUT" $ASSET_CANDIDATES <<'PY'
import json, sys, urllib.request, urllib.error
...
PY
  ...
  rc=$?
  log "[!] Не удалось получить latest release (rc=$rc). Возможно, в репозитории нет Releases." 
  write_status "failed" "check_latest" "Failed to fetch latest release" "failed to fetch latest release"
  exit 3
}
...
if ! download "$ASSET_URL" "$TARBALL" "$MAX_BYTES" "asset"; then
  log "[!] Download failed."
...
if [ -n "$SHA_URL" ]; then
  ...
  if download "$SHA_URL" "$SHA_LOCAL" "$MAX_CHECKSUM_BYTES" "checksum"; then
    VERIFY_OUT="$($PY - "$TARBALL" "$SHA_LOCAL" "$ASSET_NAME" "${SHA_KIND:-sidecar}" <<'PY'
import hashlib
import re
import sys
...
def pick_expected_from_manifest(txt: str, wanted: str) -> str:
    ...
    # If the file contains a single hash without filename, accept it.
...
tar -xzf "$TARBALL" -C "$WORK_DIR" >>"$LOG_FILE" 2>&1
