...
def test_outbounds_reports_subscription_ownership_from_state_not_jsonc_header(tmp_path, monkeypatch):
    configs_dir = tmp_path / "configs"
    jsonc_dir = tmp_path / "jsonc"
    configs_dir.mkdir()
    jsonc_dir.mkdir()
    name = "04_outbounds.json"
    main_path = configs_dir / name
    raw_path = jsonc_dir / f"{name}c"
    ...
    raw_path.write_text(
        "// Generated by XKeen UI subscription: stale header\n"
    ...
    monkeypatch.setattr(
        xray_configs_mod,
        "jsonc_path_for",
        lambda _main_path: str(raw_path),
    )
    monkeypatch.setattr(xray_configs_mod, "list_subscriptions", lambda _state_dir: [])

    app = _make_app(str(tmp_path / "state"))
    with app.test_client() as client:
        editable = client.get(f"/api/outbounds?file={name}")

    editable_body = editable.get_json()
    assert editable.status_code == 200
    assert editable_body["using_raw"] is True
    assert "Generated by XKeen UI subscription" in editable_body["text"]
    assert editable_body["managed_kind"] is None

    monkeypatch.setattr(
        xray_configs_mod,
        "list_subscriptions",
        lambda _state_dir: [{"enabled": True, "output_file": name}],
    )
    with app.test_client() as client:
        managed = client.get(f"/api/outbounds?file={name}")

    assert managed.status_code == 200
    assert managed.get_json()["managed_kind"] == "subscription"
