...
def test_stage2_header_has_two_zones_without_replacing_runtime_nodes():
    ...
    assert "filename='panel-operator.css', v='20260728e'" in template
...
def test_stage2_closure_is_reflected_in_documentation():
    ...
    for fragment in (
        "Этапы 0–2 закрыты 28 июля 2026 года; Этапы 3–7 не начаты",
    ...
    assert "Этапы 0–2 закрыты, Этапы 3–7 остаются открытыми" in index
