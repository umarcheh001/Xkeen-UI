...
Главный сторож — mtime. Обновление раскладывается без `--delete`, поэтому
однажды приехавший .gz остаётся на роутере навсегда; а правка .css прямо на
роутере по SSH — штатный способ отладки в этом проекте. Если исходник новее
.gz, сжатый файл отдавать нельзя: пользователь увидит старую панель.

Само копирование при установке этот признак не подделывает: на роутерах без
rsync install.sh копирует через `cp -r`, который времена не сохраняет, и
поэтому после копирования отдельным шагом подтягивает время каждой .gz к её
исходнику (см. tests/test_installer_precompressed_mtimes.py).
"""
