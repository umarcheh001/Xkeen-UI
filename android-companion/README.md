...
- Пункт `Zashboard UI` — optional legacy/external действие. Новые Unix-first шаблоны не открывают порт `9090` в LAN, поэтому ссылка заработает только если оператор отдельно настроил browser-reachable controller с непустым `secret` в доверенной LAN/VPN. Основные runtime-сценарии доступны во встроенной веб-панели и не требуют Zashboard.
