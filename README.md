...
### Reality и совместимость Mihomo

Xray-core начиная с **v26.7.11** при пустом `realitySettings.minClientVer`
использует минимальную версию `26.3.27`. Стандартный Mihomo сообщает в REALITY
handshake версию `1.8.2`, поэтому такой сервер может отвечать
`REALITY authentication failed`. В Xray-редакторе панель показывает это как
диагностику и предлагает quick-fix с явным `minClientVer: "0.0.0"`.

Это серверная настройка: клиентская панель не может изменить `minClientVer` у
чужой платной подписки. Для такого сервера нужно попросить провайдера указать
`minClientVer: "0.0.0"` (или совместимое значение), либо использовать клиент,
который умеет менять версию REALITY handshake, например отдельную сборку
mihomo-enhanced. Для Xray-core **v26.9.8/v26.9.9** это отдельная проблема:
сервер может требовать `X25519MLKEM768`; в Mihomo YAML-редакторе доступен
ручной quick-fix `support-x25519mlkem768: true`. Включайте его только для
новых серверов, потому что старые Reality-серверы могут быть несовместимы с
этим ClientHello.
...
В терминале показываются только параметры, основные этапы, предупреждения и итоговый адрес панели. Полный технический вывод сохраняется в `/opt/var/log/xkeen-ui-install.log`; при ошибке установщик укажет этап и путь к этому журналу.

Перед началом можно выбрать дополнительные компоненты: просмотрщик DAT (`xk-geodat`, по умолчанию устанавливается) и декриптор Happ (по умолчанию пропускается). Для запуска без вопросов используются `XKEEN_GEODAT_INSTALL=0|1` и `XKEEN_HAPP_DECRYPTOR_INSTALL=0|1`.
...
вернулся ли адрес из настроенного Fake-IP CIDR. Например, при диапазоне
`198.18.0.1/16` адрес `198.18.0.17` подтверждает Fake-IP, а не является
реальным IP. `/dns/query` может вернуть upstream IP даже при исправном Fake-IP
listener, поэтому эти два результата не смешиваются.
...
Разошедшихся на часы окно уже не сведёт, поэтому в окне подписок есть кнопка **Выровнять расписание**. Она сводит время следующего обновления всех подписок к одному моменту: за общий срок берётся медиана самой плотной группы (подписки в пределах четверти часа друг от друга считаются обновляющимися в одно время), остальные подтягиваются к ней. Подписка с ошибкой в выборе общего срока не участвует, но переезжает вместе со всеми. Периодичность (`interval_hours`) не меняется — её задаёт пользователь.

Выравнивание ничего не скачивает и не перезапускает ядро: правится только расписание. Исключение — случай, когда общий срок уже прошёл: тогда он переносится на минуту вперёд, подписки созревают вместе и ближайший тик планировщика обновляет их одной пачкой. Панель предупреждает об этом до применения. Та же кнопка есть в окне подписок Mihomo; состояния двух ядер независимы, и выравнивание для каждого своё.
