...
6. при любой ошибке возвращает YAML, состояние `dns-override` и перезапускает
   ...
   старым снимком, а отключает текущий DNS-блок на месте и возвращает DNS Keenetic.
...
| Fake-IP не работает | Проверьте `tun.enable: true` либо живые правила `mangle`/`nat` цепочки `xkeen`; удалите старое исключение `198.18.0.0/15`, затем проверьте диапазон и фильтры. |
