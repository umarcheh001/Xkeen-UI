...
В окне активации можно дополнительно включить **«Создать переключатель DNS
Proxy»**. Панель добавит отдельную группу и направит через неё защищённые DoH:
...
После активации группу `DNS Proxy` можно переключать в панели Mihomo без
повторной настройки DNS. Опция выключена по умолчанию. Если группа с таким
именем уже существует, мастер не изменяет её: выберите существующую группу в
поле маршрута или переименуйте её вручную.
...
### 5.3. Локальные и прямые DNS-зоны в окне панели

В окне **«Защищённый DNS Mihomo»** доступны те же три группы полей, что и в
DNS-over-VLESS Xray:

- **DNS-серверы в туннеле** — публичные DoH/DoT-адреса для обычных имён. Панель
  добавляет выбранную proxy-группу сама; не дописывайте к адресу `#DIRECT` или
  имя группы.
- **Локальные DNS для домашних имён** и **Домены локальной сети** — укажите
  обе части пары, например `192.168.1.2` и `lan, home.arpa`. Эти зоны попадут
  в `nameserver-policy` без прокси-маршрута.
- **DNS для доменов мимо туннеля** и **Домены прямого DNS** — тоже заполняются
  парой; панель создаёт для них `nameserver-policy` с маршрутом `DIRECT`.

Поля принимают адреса через запятую или с новой строки. Короткое имя зоны
`lan` превращается в Mihomo-правило `+.lan`, то есть охватывает и поддомены.
Одна и та же зона не может быть одновременно локальной и прямой.

Не указывайте LAN-адрес самого роутера (часто `192.168.1.1`) или
`127.0.0.1:53`: после включения `dns-override` это слушатель Mihomo и запрос
зациклится. Если нужен встроенный резолвер Keenetic для домашних имён,
используйте его отдельный служебный порт, обычно `127.0.0.1:41100`.

### 5.4. Обработка `profile.store-fake-ip`
...
маршрутизация: `tun.enable: true` либо работающий TProxy/Hybrid. Одного
ненулевого top-level `tproxy-port` недостаточно: он подтверждает слушатель
Mihomo, но не правила перехвата XKeen. Проверка для TProxy/Hybrid:
...
# Для TProxy проверьте живые правила обеих таблиц:
iptables -t mangle -S
iptables -t nat -S
```
...
Особенно проверьте, что перед `TPROXY`/`REDIRECT` нет правила вида
`-d 198.18.0.0/15 -j RETURN`: оно полностью исключает стандартный диапазон
Fake-IP `198.18.0.0/16`. Такое правило могло сохраниться при обновлении старой
установки XKeen. Если `/opt/etc/init.d/S05xkeen` имеет известный статический
формат, окно предложит действие «Исправить и включить Fake-IP»: после явного
подтверждения панель сохранит отдельную резервную копию, удалит только точный
CIDR из `ipv4_exclude`, атомарно запишет файл, перезапустит XKeen и повторно
проверит firewall. При неизвестном формате или неудачной проверке файл не
меняется либо восстанавливается из копии. Простая смена `fake-ip-range` может
...
Если блок `dns:` после включения уже отредактирован пользователем или полностью
заменён своим, не возвращайте поверх него старый снимок. Сначала сохраните текущий
файл и выключите сам слушатель Mihomo:

```yaml
dns:
  enable: false
  # остальные пользовательские поля остаются без изменений
```

Проверьте YAML, перезапустите Mihomo, а затем верните системный резолвер:
...
Одних этих двух команд недостаточно для чистого переключения: они включают DNS
Keenetic, но не заставляют Mihomo игнорировать настроенный `listen: 0.0.0.0:53`.
Без `dns.enable: false` и перезапуска процессы могут конкурировать за порт 53.
Сторож панели выполняет эту мягкую схему автоматически, когда замечает, что
`config.yaml` отличается от сохранённого им варианта: сохраняет весь текущий
блок, меняет только `dns.enable`, перезапускает ядро и выключает `dns-override`.
Полностью пользовательская конфигурация с активным слушателем `:53` и включённым
`dns-override` также попадает под наблюдение; в окне для неё доступна кнопка
«Вернуть DNS Keenetic» без необходимости иметь снимок от мастера.
...
2. для `fake-ip` проверяет TUN либо живой firewall-путь TProxy/Hybrid, включая
   отсутствие перекрывающего `RETURN`, а также диапазон и пересечения сетей;
3. запускает `mihomo -t` на временном YAML;
4. сохраняет полный снимок исходного файла и создаёт обычный backup;
5. перед передачей порта 53 отключает системный перехват транзитных DNS-запросов
   (`dns-proxy no intercept enable`) и игнорирует DNS, полученные от WAN/ISP
   (`interface <WAN> ip no name-servers` и `interface <WAN> ipv6 no name-servers`);
   исходные значения этих переключателей сохраняются в состоянии операции;
   для клиентов, которых Keenetic отправляет в отдельную DNS-политику,
   добавляет собственную цепочку `XKEEN_UI_MIHOMO_DNS` первой в `nat
   PREROUTING`, чтобы вернуть запросы к адресу роутера на Mihomo `:53`;
6. записывает новый `config.yaml`, включает `opkg dns-override`, перезапускает
   XKeen и делает UDP-запрос `example.com`;
7. при любой ошибке возвращает YAML, состояние `dns-override`, DNS-политику
   Keenetic и перезапускает
   сервис; если позже YAML был изменён пользователем, Сторож не затирает его
   старым снимком, а отключает текущий DNS-блок на месте, возвращает DNS Keenetic
   и восстанавливает исходные настройки транзита/DNS провайдера.
...
| Fake-IP не работает | Проверьте `tun.enable: true` либо живые правила `mangle`/`nat` цепочки `xkeen`; убедитесь, что `XKEEN_UI_MIHOMO_DNS` стоит первой в `nat PREROUTING`, удалите старое исключение `198.18.0.0/15`, затем проверьте диапазон и фильтры. |
